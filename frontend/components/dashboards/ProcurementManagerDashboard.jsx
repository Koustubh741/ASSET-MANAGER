import { ShoppingCart, FileText, Calendar, CreditCard, CheckCircle, Truck, XCircle, Upload, Eye, DollarSign, Download, Activity } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            ...
            <header className="flex justify-between items-end">
                <div>
                    <h1 className="text-3xl font-bold text-white light:text-slate-800">Procurement Hub</h1>
                    <p className="text-slate-400 light:text-slate-600">Manage purchasing, vendors, and budgets</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                <div className="glass-card p-5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-white light:text-slate-800">{awaitingPO.length}</h3>
                ...
                <div className="glass-card p-5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-white light:text-slate-800">{awaitingDelivery.length}</h3>
                ...
                <div className="glass-card p-5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-xs text-slate-400">Total Pending PO Value</p>
                        </div>
                    </div>
                </div>
                <div className="glass-card p-5">
                ...
                <div className="glass-card p-5">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-2xl font-bold text-white light:text-slate-800">2</h3>
            ...
            <div className="glass-panel p-6">
                <h3 className="text-lg font-bold text-white light:text-slate-800 mb-4">PO / Request trend (6 months)</h3>
                <div className="h-64">
                ...
                <div className="glass-panel p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="text-lg font-bold text-white light:text-slate-800 flex items-center gap-2">
                            ...
                            PO Intelligence
                            <span className="text-xs font-normal text-slate-400">Vendor, cost, extraction data</span>
                        </h3>
                        <button
                            onClick={() => setShowPoAudit(!showPoAudit)}
                            className="text-xs px-3 py-1.5 rounded-lg bg-white/5 border border-white/20 hover:bg-white/10"
                        >
                    ...
                    {showPoAudit && (
                        <div className="overflow-x-auto rounded-xl border border-white/10 light:border-slate-200">
                            <table className="w-full text-sm text-left">
                                <thead className="text-slate-400 light:text-slate-600 border-b border-white/10 text-xs uppercase font-bold">
                                    <tr>
                                ...
                                <tbody className="text-slate-300 divide-y divide-white/5 light:divide-slate-200">
                                    {awaitingDelivery.map(req => {
                                        const po = poDetails[req.id];
                                        const isEditing = editingPoId === req.id;
                                        return (
                                            <tr key={req.id} className="hover:bg-white/5 light:hover:bg-slate-50">
                                                <td className="p-3 font-mono text-xs">{req.id}</td>
                                                <td className="p-3">{req.assetType}</td>
                                                <td className="p-3">
                                                    {isEditing ? (
                                                        <input
                                                            className="w-full max-w-[140px] bg-slate-800 border border-white/20 rounded px-2 py-1 text-sm text-white"
                                                ...
                                                <td className="p-3 text-right">
                                                    {isEditing ? (
                                                        <input
                                                            type="number"
                                                            className="w-24 bg-slate-800 border border-white/20 rounded px-2 py-1 text-sm text-white text-right"
            ...
            <header className="flex justify-between items-end">
                <div>
                    <h1 className="text-2xl font-bold text-white light:text-slate-800 flex items-center gap-2">
                        <FileText className="text-blue-400" />
                        Purchase orders
                    </h1>
                    <p className="text-slate-400 light:text-slate-600 text-sm mt-1">Create and manage purchase orders for approved requests</p>
                </div>
            </header>
            <div className="glass-panel p-6">
                ...
                {awaitingPO.length === 0 ? (
                    <div className="p-8 text-center bg-white/5 light:bg-slate-50 rounded-xl border border-dashed border-white/10 light:border-slate-200">
                        <p className="text-slate-400 font-medium">No requests awaiting PO creation</p>
                        <p className="text-sm text-slate-500 light:text-slate-600 mt-1">Approved requests will appear here for purchase order creation.</p>
                    </div>
                ) : (
                    <div className="overflow-x-auto -mx-2 md:mx-0">
                    <table className="w-full text-sm text-left min-w-[640px] md:min-w-0">
                        <thead className="text-slate-400 light:text-slate-600 border-b border-white/10 light:border-slate-200 text-xs uppercase font-bold">
                            <tr>
                        ...
                        <tbody className="text-slate-300 divide-y divide-white/5 light:divide-slate-200">
                            {awaitingPO.map(req => (
                                <tr key={req.id} className="hover:bg-white/5 light:hover:bg-slate-50 transition-colors">
                                    <td className="py-3">
                                        <div className="font-medium text-white">{req.assetType}</div>
                                        <div className="text-xs text-slate-500 light:text-slate-600 font-mono mt-0.5">{req.id}</div>
                                    </td>
                                    <td className="py-3">
                                        <div>{req.requestedBy?.name ?? '—'}</div>
                                        <div className="text-xs text-slate-500">{req.requestedBy?.role ?? req.requestedBy?.position ?? ''}</div>
                                    </td>
                                    <td className="py-3">
                                        <div className="text-xs text-slate-400 light:text-slate-600 max-w-xs truncate">{req.justification}</div>
                                    </td>
                                    <td className="py-3">
            ...
            <header className="flex justify-between items-end">
                <div>
                    ...
                    <p className="text-slate-400 light:text-slate-600 text-sm mt-1">Confirm delivery and send to inventory</p>
                </div>
            </header>
                        ...
                        <tbody className="text-slate-300 divide-y divide-white/5 light:divide-slate-200">
                            {awaitingDelivery.map(req => (
                                <tr key={req.id} className="hover:bg-white/5 light:hover:bg-slate-50 transition-colors">
                                    <td className="py-3">
                                        <div className="font-medium text-white">{req.assetType}</div>
                                        <div className="text-xs text-slate-500 light:text-slate-600 font-mono">{req.id}</div>
                                    </td>
                                    ...
                                    <td className="py-3 text-right">
                                        <div className="flex justify-end gap-2">
                                            <button
                                                onClick={() => setSelectedRequest(req)}
                                                className="p-2 text-slate-400 light:text-slate-600 hover:text-white hover:bg-white/10 rounded-lg transition-all"
                                                title="View Details"
                                            >
                                                <Eye size={18} />
                                            </button>
            ...
            {awaitingDelivery.length === 0 && (
                <div className="glass-panel p-8 text-center">
                    ...
                    <h3 className="text-lg font-bold text-white light:text-slate-800 mb-1">Confirm Deliveries</h3>
                    <p className="text-slate-400 light:text-slate-600">No requests awaiting delivery confirmation. Finance-approved requests will appear here.</p>
                </div>
            )}
            </>
            )}
