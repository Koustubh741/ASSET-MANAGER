...
const SoftwareInstallItem = ({ app, assetId }) => {
    ...
    return (
        <div className="flex items-center justify-between p-3 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-300 dark:border-white/10 group hover:bg-slate-200 dark:bg-white/10 transition-colors">
            <div className="flex items-center gap-3">
                <div className="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-white/5 shadow-sm group-hover:bg-indigo-500/20 group-hover:text-indigo-400 group-hover:border-indigo-500/30 transition-all">
                    <Server size={14} />
                </div>
                <span className="text-sm text-slate-900 dark:text-white font-black uppercase tracking-tight">{app}</span>
            </div>
            {status === 'pending' && (
                <button
                    onClick={handleInstall}
                    className="text-xs bg-indigo-600 hover:bg-indigo-500 text-slate-900 dark:text-white px-4 py-1.5 rounded-lg shadow-[0_0_10px_rgba(79,70,229,0.3)] transition-all font-black uppercase tracking-widest"
                >
                    Install
                </button>
            )}
...
export default function ITStaffDashboard() {
    ...
    // Stockroom Stats
    const stockroomAssets = safeAssets.filter(a => a.status === ASSET_STATUS.IN_STOCK || a.status === 'STOCK' || a.status === 'STORED');
    ...
    return (
        ...
        >
            <Layout className="min-h-screen bg-slate-50 dark:bg-[#05060A] overflow-hidden relative selection:bg-rose-500/30">
                ...
                <Content className="relative z-10 p-6 lg:p-8 max-w-[2400px] mx-auto">
                    {/* --- DYNAMIC HEADER --- */}
                    <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 relative">
                        <div className="relative group">
                            ...
                            <div className="space-y-4">
                                ...
                                <Title level={1} className="text-slate-900 dark:text-white text-4xl xl:text-5xl font-black m-0 tracking-tighter leading-none font-['Outfit'] uppercase flex flex-col xl:flex-row xl:items-baseline gap-3 drop-shadow-2xl">
                                ...
                                <div className="flex items-center gap-6 pt-2">
                                    <div className="px-5 py-2.5 bg-slate-100 dark:bg-white/5 border border-slate-300 dark:border-white/10 rounded-2xl flex items-center gap-3 backdrop-blur-xl group-hover:border-rose-500/30 transition-all duration-500 shadow-2xl">
                                        ...
                                        <span className="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-[0.4em]">System Linkage: Optimized</span>
                                    </div>
                                    <span className="text-slate-500 dark:text-slate-400 text-[11px] font-black uppercase tracking-[0.3em] opacity-50 px-4 py-2 bg-slate-100 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/5 backdrop-blur-sm">NODE: {user?.full_name?.split(' ')[0].toUpperCase()} // SECURE_KEY: {user?.id?.slice(0, 8).toUpperCase()}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="relative w-full lg:w-[520px] group">
                            <div className="absolute inset-y-0 left-8 flex items-center pointer-events-none z-10 transition-transform group-focus-within:scale-110">
                                <Search size={26} className="text-slate-500 dark:text-slate-400 group-hover:text-rose-400 transition-colors" />
                            </div>
                            <input
                                type="text"
                                placeholder="Execute Universal Search Command..."
                                className="w-full h-24 bg-slate-50 dark:bg-white/[0.02] border border-slate-300 dark:border-white/10 rounded-[3rem] pl-20 pr-10 text-slate-900 dark:text-white text-lg font-black placeholder:text-slate-800 focus:outline-none focus:border-rose-500/50 focus:ring-[20px] focus:ring-rose-500/5 focus:bg-white/[0.04] backdrop-blur-3xl transition-all shadow-[inset_0_4px_30px_rgba(0,0,0,0.8)] group-hover:border-slate-300 dark:border-white/20"
                                value={searchQuery}
                    ...
                    <Divider className="my-6 border-slate-300 dark:border-white/10" />
                            ...
                            >
                                ...
                                <div className="flex justify-between items-start mb-4 relative z-10">
                                    <div className="space-y-2">
                                        <Text className="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest block">{stat.label}</Text>
                                ...
                                <div className="flex items-end justify-between relative z-10">
                                    <div className="flex flex-col">
                                        <span className={`text-4xl font-['Outfit'] font-black text-slate-900 dark:text-white group-hover:text-${stat.color}-500 dark:group-hover:text-${stat.color}-400 transition-all duration-500`}>
                                            {stat.value || 0}
                                        </span>
                                    </div>
                                    <div className={`p-3 rounded-xl bg-slate-100 dark:bg-white/5 border border-slate-300 dark:border-white/10 group-hover:border-${stat.color}-500/40 transition-all duration-500 group-hover:bg-${stat.color}-500/10`}>
                                        <stat.icon size={20} className={`text-slate-500 dark:text-slate-400 group-hover:text-${stat.color}-400 transition-all duration-500`} />
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
                        {/* MY TICKETS MODULE */}
                        <div className="lg:col-span-8 glass-panel p-4 relative overflow-hidden group border border-slate-300 dark:border-white/10">
                            ...
                            <div className="flex items-center justify-between mb-4 relative z-10">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-['Outfit'] font-black text-slate-900 dark:text-white tracking-tight">My Assigned Missions</h3>
                            ...
                            <div className="space-y-4 relative z-10">
                                {myTickets.slice(0, 5).map(ticket => (
                                    <div 
                                        key={ticket.id} 
                                        className="flex items-center justify-between p-4 bg-white dark:bg-white/[0.01] hover:bg-slate-50 dark:hover:bg-white/[0.03] border border-slate-200 dark:border-white/5 hover:border-indigo-500/40 transition-all duration-500 rounded-2xl group/ticket cursor-pointer shadow-sm relative overflow-hidden" 
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                <h4 className="text-sm font-semibold text-slate-900 dark:text-white group-hover/ticket:text-indigo-400 transition-colors line-clamp-1 mb-1">{ticket.subject}</h4>
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <span className="text-xs font-bold text-slate-500 dark:text-slate-400 flex items-center gap-2 group-hover/ticket:text-indigo-500 transition-colors uppercase tracking-tight">
                                ...
                                {myTickets.length === 0 && (
                                    <div className="text-center py-24 border border-dashed border-slate-300 dark:border-white/10 rounded-[4rem] bg-white dark:bg-white/[0.01] backdrop-blur-sm">
                                        <div className="w-20 h-20 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-full flex items-center justify-center mx-auto mb-6 shadow-2xl">
                                            <ShieldCheck size={40} className="text-slate-700 opacity-30" />
                                        </div>
                                        <p className="text-slate-500 dark:text-slate-400 font-black uppercase tracking-[0.5em] text-[11px]">All clear. Zero unresolved incidents.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-4 grid grid-cols-1 gap-8">
                            <div className="glass-panel p-8 border border-slate-300 dark:border-white/10 relative overflow-hidden group shadow-2xl">
                                ...
                                <div className="flex items-center justify-between mb-8 relative z-10">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <h3 className="text-sm font-bold text-slate-900 dark:text-white">Configuration Queue</h3>
                                    </div>
                                ...
                                <div className="space-y-4 relative z-10">
                                    {pendingQueue.map(item => (
                                        <div key={item.id} className="p-4 bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 rounded-xl hover:border-emerald-500/40 transition-all duration-300 group/item">
                                            <div className="flex justify-between items-start mb-3">
                                                <div>
                                                    <Text className="block text-sm font-bold text-slate-900 dark:text-white mb-0.5 group-hover/item:text-emerald-500 transition-colors uppercase tracking-tight">{item.name}</Text>
                                                    <div className="flex items-center gap-2">
                                    ...
                                    {pendingQueue.length === 0 && (
                                        <div className="text-center py-10">
                                            <Text type="secondary" italic className="text-[10px] uppercase font-black tracking-widest text-slate-500 dark:text-slate-400">No agents pending sync</Text>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            {/* STOCKROOM SNAPSHOT */}
                            <div className="glass-panel p-8 border border-slate-300 dark:border-white/10 overflow-hidden relative group shadow-2xl">
                                ...
                                <div className="flex items-center justify-between mb-8 relative z-10">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <h3 className="text-sm font-bold text-slate-900 dark:text-white">Global Assets</h3>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center justify-between p-6 bg-sky-500/10 rounded-3xl border border-sky-500/20 mb-8 relative z-10 shadow-sm dark:shadow-inner overflow-hidden">
                                    ...
                                    <div className="flex flex-col relative z-20">
                                        ...
                                        <Title level={2} className="!m-0 !text-xl font-['Outfit'] font-black text-slate-900 dark:text-white drop-shadow-md">{stockroomAssets.length}</Title>
                                    </div>
                                    <Activity className="text-sky-500/30 animate-pulse" size={40} />
                                </div>

                                <div className="grid grid-cols-2 gap-4 mb-8 relative z-10">
                                    <div className="p-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-2xl group/subitem hover:border-sky-500/30 transition-all">
                                        <Text className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-black tracking-widest mb-1 group-hover/subitem:text-sky-400 transition-colors">Laptops</Text>
                                        <Title level={3} className="!m-0 text-slate-900 dark:text-white font-['Outfit'] font-black">{stockroomAssets.filter(a => a.type?.toLowerCase() === 'laptop').length}</Title>
                                    </div>
                                    <div className="p-4 bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5 rounded-2xl group/subitem hover:border-sky-500/30 transition-all">
                                        <Text className="text-[10px] text-slate-500 dark:text-slate-400 block uppercase font-black tracking-widest mb-1 group-hover/subitem:text-sky-400 transition-colors">Infra Nodes</Text>
                                        <Title level={3} className="!m-0 text-slate-900 dark:text-white font-['Outfit'] font-black">{stockroomAssets.filter(a => a.type?.toLowerCase() === 'monitor' || a.type?.toLowerCase() === 'server').length}</Title>
                                    </div>
                                </div>
                                <div className="relative z-10 border-t border-slate-300 dark:border-white/10 pt-6">
                                    <Text className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.3em] mb-4 block">Deployment Log</Text>
                                    <div className="space-y-3">
                                        {stockroomAssets.slice(0, 3).map(asset => (
                                            <div key={asset.id} className="flex justify-between items-center bg-slate-50 dark:bg-white/[0.02] p-3 rounded-xl border border-slate-200 dark:border-white/5 hover:bg-slate-100 dark:bg-white/[0.05] transition-all">
                                                <Text className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-400 font-black uppercase tracking-tight line-clamp-1">{asset.name}</Text>
                    ...
                    <div className="mt-20">
                        ...
                        <div className="glass-panel p-6 lg:p-10 border border-slate-300 dark:border-white/10 relative overflow-hidden group shadow-[0_20px_50px_rgba(0,0,0,0.1)] dark:shadow-[0_40px_100px_rgba(0,0,0,0.5)]">
                            ...
                            <div className="flex items-center justify-between mb-16 relative z-10">
                                <div>
                                    <h3 className="text-2xl font-['Outfit'] font-black text-slate-900 dark:text-white tracking-tight uppercase m-0 leading-none flex items-center gap-4">
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-6 ml-[4rem] border-l-2 border-rose-500/30 pl-6">Global Incident Pipeline // Sync Active</p>
                                </div>
                                <div className="flex items-center gap-8">
                                    <div className="flex -space-x-3">
                                        {[1,2,3,4].map(i => (
                                            <div key={i} className={`w-10 h-10 rounded-full border-2 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-400 transition-all duration-500 hover:-translate-y-1 hover:scale-105 cursor-help ${i === 4 ? 'bg-rose-500/20 text-rose-500 border-rose-500/30' : ''}`}>
                            ...
                            <div className="space-y-8 relative z-10">
                                {Object.entries(unassignedTicketsByCategory).map(([category, catTickets]) => (
                                    <div key={category} className="space-y-8">
                                        <div className="flex items-center gap-10">
                                            ...
                                            <div className="flex items-center gap-4 px-6 py-2 rounded-full bg-slate-50 dark:bg-white/[0.02] border border-slate-200 dark:border-white/5 shadow-xl backdrop-blur-3xl group/cat">
                                                <div className="w-2 h-2 rounded-full bg-rose-500 animate-pulse"></div>
                                                <h4 className="text-xs font-bold uppercase tracking-widest text-slate-600 dark:text-slate-400 m-0 group-hover/cat:text-rose-500 transition-colors">
                                                    {category}
                                                </h4>
                                            ...
                                            {catTickets.map((ticket, index) => {
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <h4 className="text-base font-bold text-slate-900 dark:text-white group-hover/ticket:text-rose-500 transition-colors mb-3 line-clamp-2 leading-tight">{ticket.subject}</h4>
                                                        
                                                        <div className="mb-6 space-y-3 relative z-10">
                                                            <div className="flex items-center gap-3 text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                                                                <div className="p-1.5 bg-slate-100 dark:bg-white/5 rounded-lg border border-slate-200 dark:border-white/5">
                                                        ...
                                                        <div className="flex items-center justify-between mt-auto pt-6 border-t border-slate-100 dark:border-white/5 relative z-10">
                                                            <div className="flex items-center gap-4">
                                                                <div className="relative">
                                                                ...
                                                                <div className="flex flex-col">
                                                                    <span className="text-sm text-slate-900 dark:text-slate-100 font-bold leading-none">{ticket.requestor_name || 'Anonymous User'}</span>
                                ...
                                {unassignedTickets.length === 0 && (
                                    <div className="text-center py-16 bg-slate-50 dark:bg-white/[0.01] rounded-2xl border border-dashed border-slate-200 dark:border-white/5">
                                        <Zap size={32} className="text-slate-400 mx-auto mb-4 opacity-30" />
                                        <p className="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-xs">Registry is clean. No anomalies detected.</p>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {activeModal === 'CONFIG' && selectedItem && (
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-xl animate-in fade-in duration-500">
                            <div className="bg-white dark:bg-slate-900/90 border border-slate-300 dark:border-white/10 rounded-[3rem] w-full max-w-xl overflow-hidden shadow-[0_50px_100px_rgba(0,0,0,0.8)] animate-in zoom-in-95 duration-500 relative">
                                ...
                                <div className="p-12 flex justify-between items-center relative z-10">
                                    <div className="flex items-center gap-8">
                                        ...
                                        <div>
                                            <h3 className="text-2xl font-['Outfit'] font-black text-slate-900 dark:text-white tracking-tighter uppercase m-0 leading-none">Process Initialization</h3>
                                            <p className="text-[11px] text-slate-500 dark:text-slate-400 font-black uppercase tracking-[0.5em] mt-5 flex items-center gap-3 border-l-2 border-emerald-500/30 pl-6">
                                    ...
                                    <button
                                        ...
                                        className="p-4 bg-slate-100 dark:bg-white/5 hover:bg-rose-500/10 rounded-[1.5rem] text-slate-500 dark:text-slate-400 hover:text-rose-400 transition-all border border-slate-200 dark:border-white/5 group"
                                    >
                                ...
                                <div className="px-12 pb-12 space-y-10 relative z-10">
                                    {configStep === 1 && (
                                        <div className="text-center py-20 bg-slate-50 dark:bg-white/[0.02] rounded-[3.5rem] border border-slate-200 dark:border-white/5 shadow-2xl relative overflow-hidden group">
                                            ...
                                            <h4 className="text-2xl font-black text-slate-900 dark:text-white mb-4 uppercase tracking-[0.4em]">Security Matrix Alignment</h4>
                                            <p className="text-xs text-slate-500 dark:text-slate-400 max-w-[320px] mx-auto font-black uppercase tracking-widest leading-loose italic opacity-70">Verifying TPM 2.0 signatures and Secure Boot chain of trust // Protocol Alpha Active.</p>
                                        </div>
                                    )}
                                    {configStep === 2 && (
                                        <div className="space-y-6">
                                            <div className="flex items-center justify-between mb-8 px-4">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <h4 className="text-xs font-black uppercase tracking-[0.6em] text-slate-500 dark:text-slate-400 dark:text-slate-400">Registry Propagation Sequence</h4>
                                                </div>
                                            ...
                                            <div className="space-y-4">
                                                {[
                                                    ...
                                                    <div key={idx} className={`p-8 bg-white dark:bg-white/[0.01] border rounded-[2.5rem] flex items-center justify-between transition-all duration-500 group/sw
                                                        ${sw.active ? 'border-emerald-500/40 bg-emerald-500/5 shadow-2xl' : 'border-slate-200 dark:border-white/5 hover:border-slate-300 dark:border-white/20'}`}>
                                                        <div className="flex items-center gap-6">
                                                            ...
                                                            <span className={`text-sm font-black uppercase tracking-widest transition-colors ${sw.active || sw.status === 'Linked' ? 'text-slate-900 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}>{sw.label}</span>
                                                        </div>
                                    ...
                                    {configStep === 3 && (
                                        <div className="text-center py-20 bg-emerald-500/5 rounded-[4rem] border border-emerald-500/20 shadow-[0_0_100px_rgba(16,185,129,0.15)] relative overflow-hidden group">
                                            ...
                                            <h4 className="text-xl font-bold text-slate-900 dark:text-white mb-4 uppercase tracking-wider">Node Sync Successful</h4>
                                    ...
                                    <div className="pt-10 border-t border-slate-200 dark:border-white/5 mt-10 relative z-10">
                                        <button
                                        ...
                                        <div className="flex justify-center gap-4 mt-12">
                                            {[1, 2, 3].map(s => (
                                                <div key={s} className={`h-1.5 w-12 rounded-full transition-all duration-1000 ${configStep >= s ? 'bg-emerald-400 shadow-[0_0_15px_rgba(52,211,153,0.8)]' : 'bg-slate-100 dark:bg-white/5'}`}></div>
                                            ))}
                                        </div>
                                    </div>
                                </div>
                    ...
                    {activeModal === 'RESOLVE_TICKET' && selectedItem && (
                        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-50 dark:bg-slate-950/80 backdrop-blur-2xl animate-in fade-in duration-500">
                        <div className="bg-white dark:bg-slate-900/90 border border-slate-200 dark:border-white/10 rounded-2xl w-full max-w-2xl overflow-y-auto max-h-[90vh] shadow-2xl animate-in zoom-in-95 duration-500 relative">
                                <div className="absolute top-0 right-0 w-[30rem] h-[30rem] bg-rose-500/10 blur-[150px] rounded-full pointer-events-none"></div>
                                
                                <div className="p-6 flex justify-between items-center relative z-10 border-b border-slate-100 dark:border-white/5">
                                    <div className="flex items-center gap-8">
                                        ...
                                        <div>
                                            <h3 className="text-2xl font-['Outfit'] font-black text-slate-900 dark:text-white tracking-tight leading-none">Mission Finalization</h3>
                                            <p className="text-sm text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest mt-4 flex items-center gap-3 border-l-2 border-rose-500/30 pl-4">
                                                <span className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.8)]"></span>
                                                Encryption_Key: <span className="text-slate-900 dark:text-white font-mono">{formatId(selectedItem.id, 'ticket', selectedItem)}</span>
                                            </p>
                                        </div>
                                    </div>
                                    <button
                                ...
                                <div className="px-6 pb-6 space-y-5 relative z-10">
                                    <div className="bg-slate-50 dark:bg-white/[0.02] p-10 rounded-[3rem] border border-slate-200 dark:border-white/5 shadow-2xl relative overflow-hidden group">
                                        ...
                                        <div className="flex items-center gap-3 mb-4">
                                            ...
                                            <p className="text-xs font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Objective Narrative Protocol</p>
                                        </div>
                                        <p className="text-slate-900 dark:text-white font-black text-2xl tracking-tight leading-tight mb-6">{selectedItem.subject}</p>
                                        <div className="flex items-center gap-4">
                                            ...
                                            <span className="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-wide flex items-center gap-2">
                                    ...
                                    <div>
                                        <div className="flex items-end justify-between mb-8 px-4">
                                            <div className="space-y-1">
                                                <h4 className="text-base font-bold text-slate-900 dark:text-white">Resolution Checklist</h4>
                                            ...
                                            <div className="text-right">
                                                ...
                                                <div className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wide">Ready for Deployment</div>
                                            </div>
                                        </div>
                                        
                                        <div className="h-2 bg-slate-100 dark:bg-white/[0.05] rounded-full overflow-hidden border border-slate-200 dark:border-white/5 mb-5 relative">
                                            <div
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                ...
                                                <div 
                                                    ...
                                                    className={`flex items-center justify-between p-6 rounded-[1.75rem] border transition-all duration-500 cursor-pointer group/item
                                                        ${item.checked 
                                                            ...
                                                            : 'bg-white dark:bg-white/[0.01] border-slate-200 dark:border-white/5 hover:border-slate-300 dark:border-white/20'}`}
                                                >
                                                    <div className="flex items-center gap-5">
                                                        <div className={`w-7 h-7 rounded-[0.75rem] border flex items-center justify-center transition-all duration-500
                                                            ${item.checked 
                                                                ...
                                                                : 'bg-transparent border-slate-300 dark:border-white/10 group-hover/item:border-rose-500/50'}`}>
                                                            {item.checked && <Check size={16} className="text-slate-900 dark:text-white font-black" />}
                                                        </div>
                                                        <span className={`text-sm font-bold uppercase tracking-tight transition-all pb-0.5 ${item.checked ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400 group-hover/item:text-slate-900 dark:group-hover/item:text-white'}`}>{item.text}</span>
                                                    </div>
                                                    <button 
                                            ...
                                            <div className="flex gap-2 group/input col-span-1 md:col-span-2 mt-4">
                                                <input
                                                    ...
                                                    className="flex-1 min-h-[44px] bg-slate-50 dark:bg-white/[0.02] border border-slate-300 dark:border-white/10 rounded-xl px-4 text-sm font-medium text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:border-rose-500/40 transition-all"
                                    ...
                                    <div className="space-y-6">
                                        <div className="flex items-center gap-4 ml-1">
                                            <div className="w-1 h-4 bg-rose-500/50 rounded-full"></div>
                                             <Text className="text-base font-bold text-slate-900 dark:text-white block">Resolution Notes</Text>
                                        </div>
                                        <textarea
                                            className="w-full h-48 bg-white dark:bg-white/[0.01] border border-slate-200 dark:border-white/5 rounded-[3rem] p-10 text-slate-900 dark:text-white text-base font-bold placeholder:text-slate-800 focus:outline-none focus:border-rose-500/40 focus:ring-[20px] focus:ring-rose-500/5 focus:bg-slate-50 dark:bg-white/[0.02] transition-all shadow-sm dark:shadow-inner custom-scrollbar backdrop-blur-3xl"
                                    ...
                                    <div className="flex gap-8 pt-8 relative z-10">
                                        <button
                                            onClick={() => setActiveModal(null)}
                                            className="px-8 py-5 rounded-xl border border-slate-200 dark:border-white/10 text-slate-600 dark:text-slate-400 text-sm font-bold uppercase tracking-widest hover:bg-slate-100 dark:hover:bg-white/5 transition-all shadow-lg active:scale-95"
                                        >
                                            Cancel
                                        </button>
                                        <button
