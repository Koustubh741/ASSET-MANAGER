...
const RenderStructuredData = ({ data, onValueChange, editing = false }) => {
    if (!data || typeof data !== 'object') return <span className="text-app-text-muted italic">No extraction data available</span>;
    ...
    const getIcon = (key) => {
        ...
        if (k.includes('id') || k.includes('number')) return <Hash size={14} className="text-app-text-muted" />;
        return <Activity size={14} className="text-indigo-400" />;
    };

    const getConfidenceColor = (value) => {
        const val = parseFloat(value);
        if (isNaN(val)) return 'text-app-text-muted';
                    ...
                    <div key={key} className={`${isLongText ? 'col-span-full' : ''} group relative px-4 py-3 rounded-xl bg-white dark:bg-white/[0.01] dark:bg-white/[0.01] border border-slate-200 dark:border-white/20 hover:bg-slate-50 dark:hover:bg-slate-50 dark:bg-white/[0.03] transition-all duration-300 shadow-sm hover:shadow-md`}>
                        <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-2">
                                {getIcon(key)}
                                <span className="text-[10px] font-black text-app-text-muted uppercase tracking-widest">{displayKey}</span>
                            </div>
                        ...
                        {editing && !isConfidence ? (
                            ...
                            isLongText ? (
                                ...
                                <div className={`text-lg font-black tracking-tight ${isConfidence ? getConfidenceColor(value) : 'text-app-text'}`}>
...
export default function FinanceDashboard({ activeView: initialView = 'dashboard' }) {
    ...
    return (
        <div className="space-y-6 neural-compact">
            {showDashboard && (
                <header className="flex flex-col lg:flex-row lg:items-center justify-between bg-white dark:bg-slate-900/40 backdrop-blur-3xl p-8 rounded-[32px] border border-app-border shadow-2xl relative overflow-hidden group">
                    ...
                    <div className="relative z-10 mb-6 lg:mb-0 text-center lg:text-left">
                        <h2 className="text-2xl font-black text-app-text tracking-tighter flex flex-col lg:flex-row lg:items-center gap-3">
                        ...
                        <p className="text-app-text-muted text-[10px] font-black uppercase tracking-[0.4em] mt-3 lg:ml-5 flex items-center justify-center lg:justify-start gap-2">
            ...
            {showBudgetQueue && (
                <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-8 px-4">
                    <div>
                        <h1 className="text-2xl font-black text-app-text tracking-tighter flex items-center gap-4">
                        ...
                        <p className="text-app-text-muted text-[10px] font-black uppercase tracking-[0.4em] mt-3 ml-6 flex items-center gap-2">
            ...
            {showDashboard && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="group relative glass-panel p-8 overflow-hidden border border-app-border shadow-lg hover:shadow-2xl transition-all duration-500 rounded-[32px]">
                        ...
                        <p className="text-xl font-black text-app-text tracking-tighter">₹{(ytdDepreciation / 100000).toFixed(1)}<span className="text-xl text-app-text-muted ml-1 font-bold">Lacs</span></p>
                        <p className="text-[10px] text-app-text-muted mt-4 uppercase font-black italic tracking-widest">Straight-line projection ACTIVE</p>
                    </div>
                    ...
                    <div className="group relative glass-panel p-8 overflow-hidden border border-app-border shadow-lg hover:shadow-2xl transition-all duration-500 rounded-[32px]">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex -space-x-2">
                                {[1, 2, 3].map(i => (
                                    <div key={i} className="w-8 h-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[8px] text-app-text-muted font-black shadow-sm">
                                        JD
                                    </div>
                                ))}
                            </div>
                        </div>
                        <p className="text-xl font-black text-app-text tracking-tighter">{budgetApprovals.length}<span className="text-xl text-app-text-muted ml-1 font-bold">Files</span></p>
            ...
            {showDashboard && showPoTable && (
                <div className="glass-panel p-8 border border-app-border shadow-3xl animate-in fade-in slide-in-from-bottom-8 duration-700 relative overflow-hidden rounded-[32px]">
                    <div className="absolute inset-0 bg-gradient-to-br from-indigo-500/[0.02] to-transparent pointer-events-none"></div>
                    <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-slate-100 border-app-border pb-8 mb-8 relative z-10 gap-4">
                        <div>
                            <h3 className="text-xl font-black text-app-text tracking-tight flex items-center gap-3">
                    ...
                    <div className="overflow-hidden rounded-3xl border border-app-border bg-white dark:bg-slate-950/40 shadow-2xl">
                        <div className="overflow-x-auto">
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="bg-slate-50/50 dark:bg-white/[0.02] border-b border-slate-100 border-app-border">
                                        <th className="p-6 text-[10px] font-black text-app-text-muted uppercase tracking-widest">Node ID</th>
                                        <th className="p-6 text-[10px] font-black text-app-text-muted uppercase tracking-widest">Vendor Context</th>
                                        <th className="p-6 text-[10px] font-black text-app-text-muted uppercase tracking-widest text-right">Extracted Fiscal</th>
                                        <th className="p-6 text-[10px] font-black text-app-text-muted uppercase tracking-widest text-center">Batch</th>
                                        <th className="p-6 text-[10px] font-black text-app-text-muted uppercase tracking-widest">Auth Status</th>
                                        <th className="p-6 text-[10px] font-black text-app-text-muted uppercase tracking-widest text-right">Tracing</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-slate-100 dark:divide-white/5 text-app-text">
                                    {budgetApprovals.map(req => {
                                        const po = poDetails[req.id];
                                        return (
                                            <tr key={req.id} className="group hover:bg-slate-50 dark:hover:bg-slate-50 dark:bg-white/[0.02] transition-colors border-b border-slate-100 border-app-border last:border-0 font-medium">
                                                ...
                                                <td className="p-6 text-sm font-black text-app-text text-right font-mono">
                                                    {po?.total_cost != null ? `₹${Number(po.total_cost).toLocaleString()}` : '--'}
                                                </td>
                                                <td className="p-6 text-sm text-app-text-muted text-center font-black">{po?.quantity ?? '0'}</td>
                                                <td className="p-6">
                                                    ...
                                                    {po?.extracted_data ? (
                                                        <details className="group/details">
                                                            ...
                                                            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-12 pointer-events-none">
                                                                <div className="bg-white dark:bg-slate-900 border border-app-border rounded-[32px] p-8 shadow-4xl max-w-4xl w-full pointer-events-auto max-h-[80vh] overflow-auto animate-in zoom-in-95 duration-300">
                                                                    <div className="flex items-center justify-between mb-8 border-b border-slate-100 border-app-border pb-4">
                                                                        <h4 className="text-xl font-black text-app-text uppercase tracking-tighter">Diagnostic Telemetry: {req.id}</h4>
                                                        ...
                                                        <span className="text-[10px] font-black text-app-text-muted uppercase tracking-widest italic">Silent Node</span>
                                                    )}
                                                </td>
                                            </tr>
                                        );
                        ...
                        {budgetApprovals.map(req => {
                            ...
                            return (
                                <div key={req.id} className="relative group/req-card">
                                    ...
                                    <div className="relative glass-panel rounded-[36px] p-10 border border-app-border shadow-3xl overflow-hidden hover:shadow-emerald-500/10 transition-all duration-500">
                                        ...
                                        <div className="flex flex-col lg:flex-row gap-16 relative z-10">
                                            ...
                                            <div className="flex-1 space-y-10">
                                                <div className="flex items-start gap-8">
                                                    <div className="w-20 h-20 rounded-[24px] bg-app-surface-soft border border-app-border flex items-center justify-center text-indigo-500 shadow-sm dark:shadow-inner group-hover/req-card:scale-110 transition-transform duration-700">
                                                        <Activity size={36} />
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="grid grid-cols-2 gap-12 py-8 border-y border-slate-100 border-app-border">
                                                    <div className="space-y-3">
                                                        <div className="flex items-center gap-2 text-[10px] font-black text-app-text-muted uppercase tracking-widest">
                                                            <User size={14} className="text-indigo-500" /> Originator
                                                        </div>
                                                        <p className="text-xl font-black text-app-text uppercase tracking-tight">{req.requestedBy.name}</p>
                                                    </div>
                                                    <div className="space-y-3">
                                                        <div className="flex items-center gap-2 text-[10px] font-black text-app-text-muted uppercase tracking-widest">
                                                            <PieChart size={14} className="text-emerald-500" /> Dept / Unit
                                                        </div>
                                                        <p className="text-xl font-black text-app-text uppercase tracking-tight">{req.requestedBy.role}</p>
                                                    </div>
                                                </div>

                                                <div className="space-y-4">
                                                    <div className="text-[10px] font-black text-app-text-muted uppercase tracking-[0.3em] ml-1">Capital Justification</div>
                                                    <div className="relative">
                                                        <div className="absolute left-0 top-0 bottom-0 w-1 bg-indigo-500 rounded-full opacity-30 shadow-lg"></div>
                                                        <p className="text-lg text-app-text-muted leading-relaxed font-medium italic pl-8 py-2 bg-slate-50/50 dark:bg-white/[0.02] rounded-r-3xl border-r border-slate-100 border-app-border">
                                                            "{req.justification}"
                                                        </p>
                                                    </div>
                                                </div>
                                            ...
                                            <div className="lg:w-[520px] bg-slate-50 dark:bg-black/20 rounded-[32px] p-8 border border-slate-100 border-app-border shadow-sm dark:shadow-inner flex flex-col justify-between">
                                                {po ? (
                                                        ...
                                                        <div className="grid grid-cols-2 gap-6">
                                                            <div className="bg-white dark:bg-white/[0.03] p-5 rounded-3xl border border-slate-100 border-app-border shadow-sm">
                                                                ...
                                                                {editingPos[req.id] ? (
                                                                    ...
                                                                    <div className="text-xl font-black text-app-text uppercase tracking-tight truncate">{po.vendor_name || 'NOT FOUND'}</div>
                                                                )}
                                                            </div>
                                                            ...
                                                            <div className="bg-white dark:bg-white/[0.03] p-6 rounded-[32px] border border-slate-100 border-app-border shadow-sm col-span-2 space-y-4">
                                                                <div className="flex items-center justify-between text-[10px] font-black text-app-text-muted uppercase tracking-widest">
                                                                ...
                                                                <div className="grid grid-cols-3 gap-6">
                                                                    <div>
                                                                        ...
                                                                        <span className="text-sm font-black text-app-text uppercase">{po.capex_opex || 'OPEX'}</span>
                                                                    </div>
                                                                    <div>
                                                                        <label className="text-[9px] font-black text-emerald-500 uppercase tracking-widest block mb-2">Tax Accrual</label>
                                                                        <span className="text-sm font-black text-app-text uppercase">₹{(po.tax_amount || 0).toLocaleString()}</span>
                                                                    </div>
                                                                    <div>
                                                                        <label className="text-[9px] font-black text-amber-500 uppercase tracking-widest block mb-2">Logistics</label>
                                                                        <span className="text-sm font-black text-app-text uppercase">₹{(po.shipping_handling || 0).toLocaleString()}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                        ...
                                        {expandedPoLogs[req.id] && (
                                            <div className="mt-12 pt-12 border-t border-slate-100 border-app-border animate-in slide-in-from-top-12 duration-500">
                                                <div className="flex items-center gap-4 mb-8">
                                                    <div className="w-1.5 h-6 bg-indigo-500 rounded-full"></div>
                                                    <h5 className="text-[12px] font-black text-app-text uppercase tracking-[0.3em]">Payload Structure Verification</h5>
                                                </div>
            ...
            {showDashboard && (
                <div className="glass-panel p-8 border border-app-border shadow-lg rounded-[32px]">
