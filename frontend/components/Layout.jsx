...
import { LayoutDashboard, Server, Settings, User, RotateCcw, ShoppingBag, Trash2, Sparkles, Monitor, MapPin, ChevronUp, Check, Cpu, Network, DollarSign, Menu, X } from 'lucide-react'
...
export default function Layout({ children }) {
    ...
    return (
        <div className="min-h-screen flex text-slate-100 font-sans">
            ...
            <a
                href="#main-content"
                className="sr-only focus:not-sr-only focus:absolute focus:top-4 focus:left-4 focus:z-[100] focus:px-4 focus:py-2 focus:rounded-lg focus:bg-indigo-600 focus:text-white focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-slate-950 focus:outline-none"
            >
                Skip to main content
            </a>

            {/* Mobile Header - Sticky, visible on small screens */}
            <header className="md:hidden fixed top-0 left-0 right-0 z-30 h-14 bg-slate-900/95 backdrop-blur-xl border-b border-white/10 flex items-center justify-between px-4">
                ...
                <button
                    ...
                    className="p-2.5 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 focus:outline-none focus:ring-2 focus:ring-indigo-500 min-h-[44px] min-w-[44px] flex items-center justify-center"
            ...
            <div className={`md:hidden fixed inset-0 z-20 transition-opacity duration-200 ${mobileMenuOpen ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
                ...
                <aside className={`absolute top-14 left-0 right-0 bottom-0 bg-slate-900/98 backdrop-blur-xl border-r border-white/10 transform transition-transform duration-200 ease-out ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                    <nav className="p-4 space-y-1 overflow-y-auto pt-6">
                        {navItems.map((item) => {
                            ...
                            return (
                                <Link
                                    ...
                                    className={`flex items-center gap-3 px-4 py-3 rounded-xl min-h-[44px] transition-all ${isActive ? 'bg-blue-600/20 text-blue-300 border border-blue-500/30' : 'text-slate-300 hover:bg-white/5'}`}
                                >
                                    <Icon size={20} />
                        ...
                        <div className="mt-6 pt-4 border-t border-white/10">
                            <button
                                ...
                                className="w-full flex items-center gap-3 px-4 py-3 rounded-xl min-h-[44px] border border-rose-500/20 text-rose-400 hover:bg-rose-500/10 font-medium"
                            >
                                Log Out
                            </button>
                        </div>
                            ...
                            {navItems.map((item) => {
                                ...
                                return (
                                    <Link
                                        key={item.href}
                                        href={item.href}
                                        className={`flex items-center space-x-3 px-4 py-3.5 rounded-xl transition-all duration-300 whitespace-nowrap focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-slate-900 ${isActive
                        ...
                        <div className="relative whitespace-nowrap">
                            ...
                            <div className="mx-4 mb-4">
                                <button
                                    ...
                                    className="w-full py-2 rounded-lg border border-rose-500/20 text-rose-400 text-xs font-medium hover:bg-rose-500/10 hover:text-rose-300 transition-colors flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-slate-900"
                                >
                                    Log Out
                                </button>
                            </div>
            ...
            {/* Main Content */}
            <main id="main-content" className="flex-1 md:ml-28 pt-20 md:pt-0 p-6 md:p-8 animate-in fade-in duration-700 overflow-auto">
                {/* Getting started onboarding - dismissed via localStorage */}
                {showOnboarding && (
                    <div className="mb-6 p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-between gap-4 animate-in slide-in-from-top-4 fade-in duration-500">
                        <p className="text-sm text-slate-200">
                            <span className="font-semibold text-indigo-300">Getting started:</span> Request an asset from your dashboard or view your tickets.
                        </p>
                        <button
                            onClick={dismissOnboarding}
                            className="shrink-0 px-3 py-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/10 text-xs font-medium transition-colors"
                        >
                            Dismiss
                        </button>
                    </div>
                                ...
                                if (notifications?.system) {
                                    return (
                                        <div className="mb-6 p-4 rounded-xl bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 flex items-center justify-between animate-in slide-in-from-top-4 fade-in duration-500">
                                            ...
                                            <button className="px-3 py-1.5 rounded-lg bg-blue-600 text-white text-xs font-medium hover:bg-blue-700 transition-colors shadow-lg shadow-blue-500/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 focus:ring-offset-slate-900">
                                                View Details
                                            </button>
                                        </div>
                                    )
                ...
                {/* Footer */}
                <footer className="mt-12 pt-6 border-t border-white/5 text-center text-xs text-slate-500">
                    Asset Manager v1.0
                </footer>
            </main>
        </div>
