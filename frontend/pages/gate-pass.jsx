        ...
        <>
            ...
            <div className="space-y-6">
                <header className="flex justify-between items-center flex-wrap gap-4">
                    <div>
                        <h1 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">🚪 Gate Pass</h1>
                        <p className="text-slate-500 dark:text-slate-400 mt-1">Authorize physical movement of assets</p>
                    </div>
                    <button
                ...
                {showForm && (
                    <form onSubmit={createPass} className="glass-card p-6 space-y-4">
                        <h2 className="text-lg font-semibold text-slate-900 dark:text-white">New Gate Pass</h2>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full bg-slate-100 dark:bg-white/5 border border-white/15 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white"
                                >
                            ...
                            <div>
                                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Issued To *</label>
                                <input
                                    required
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Destination</label>
                                <input
                                    type="text"
                            ...
                            <div>
                                <label className="block text-xs text-slate-500 dark:text-slate-400 mb-1">Valid Until</label>
                                <input
                                    type="datetime-local"
                            ...
                            <div className="md:col-span-2">
                                ...
                                <textarea
                                    ...
                                    className="w-full bg-slate-100 dark:bg-white/5 border border-white/15 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white resize-none h-20"
                                />
                            </div>
                        </div>
                        <div className="flex gap-3 justify-end">
                            <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2 rounded-lg text-sm text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:text-white">Cancel</button>
                ...
                {loading ? (
                    <div className="glass-card p-8 text-center text-slate-500 dark:text-slate-400">Loading gate passes…</div>
                ) : passes.length === 0 ? (
                    <div className="glass-card p-12 text-center">
                        <div className="text-xl mb-4">🚪</div>
                        <p className="text-slate-500 dark:text-slate-400">No gate passes yet. Create one above.</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {passes.map(gp => {
                                    ...
                                    return (
                                        <tr key={gp.id} className="border-b border-slate-200 dark:border-white/5 hover:bg-white/3 transition-colors">
                                            <td className="px-4 py-3 text-slate-900 dark:text-white font-medium">{gp.asset_name || gp.asset_id?.slice(0, 8)}</td>
                                            <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{gp.issued_to}</td>
                                            <td className="px-4 py-3 text-slate-700 dark:text-slate-300">{gp.issued_by}</td>
                                            <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{gp.destination || '—'}</td>
