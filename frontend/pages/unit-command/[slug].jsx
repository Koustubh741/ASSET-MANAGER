...
const SLA_CONFIG = {
    MET:         { label: 'Met',         color: 'text-green-400',  bg: 'bg-green-500/10 border-green-500/20' },
    BREACHED:    { label: 'Breached',    color: 'text-red-400',    bg: 'bg-red-500/10 border-red-500/30' },
    IN_PROGRESS: { label: 'On Track',    color: 'text-blue-400',   bg: 'bg-blue-500/10 border-blue-500/20' },
    WARNING:     { label: 'Critical',    color: 'text-orange-400', bg: 'bg-orange-500/10 border-orange-500/30' },
};
...
function renderSLABadge(ticket) {
    if (!ticket.sla_resolution_deadline) return <span className="text-[10px] text-app-text-muted/30 italic">No SLA</span>;
    ...
    // Status Logic
    let status = ticket.sla_resolution_status || 'IN_PROGRESS';
    
    // Real-time Override: If backend says IN_PROGRESS but deadline passed
    if (status === 'IN_PROGRESS' && !isResolved && now > deadline) {
        status = 'BREACHED';
    }

    // Warning: Less than 2 hours left
    const diffHours = (deadline - now) / (1000 * 60 * 60);
    if (status === 'IN_PROGRESS' && diffHours < 2 && diffHours > 0) {
        status = 'WARNING';
    }

    const cfg = SLA_CONFIG[status] || SLA_CONFIG.IN_PROGRESS;

    return (
        <div className="flex flex-col gap-1">
            <span className={`px-2 py-0.5 text-[8px] font-black uppercase border tracking-widest inline-flex w-fit ${cfg.color} ${cfg.bg}`}>
                {cfg.label}
            </span>
            {status !== 'BREACHED' && !isResolved && (
                <span className={`text-[9px] font-mono ${status === 'WARNING' ? 'text-orange-400 animate-pulse' : 'text-app-text-muted'}`}>
                    {diffHours > 24 ? `${Math.floor(diffHours/24)}d left` : `${Math.floor(diffHours)}h left`}
                </span>
            )}
        </div>
    );
...
export default function DepartmentPortal() {
    ...
    const loadTickets = useCallback(async () => {
        ...
        try {
            ...
            // ROOT FIX: Support roles & Admins pass the dept name as a filter to override home-dept scoping
            if ((isAdmin || currentRole?.slug === 'SUPPORT') && dept?.name) {
                params.department = dept.name;
            }
            if (search.trim()) params.search = search.trim();
            const data = await apiClient.get(`/tickets/?${new URLSearchParams(params)}`);
            // ROOT FIX: Handle PaginatedResponse (extract .data array and total count)
            const ticketList = data.data || (Array.isArray(data) ? data : []);
            setTickets(ticketList);
            setTotalTickets(data.total || ticketList.length);
    ...
    }, [slug, isStaff, dept, search]);

    // Invalid slug guard
    useEffect(() => {
        if (slug && (slug === 'undefined' || slug === 'null')) {
            router.replace('/unit-command');
        }
    }, [slug, router]);
    ...
    const stats = {
        ...
        breached:   tickets.filter(t => {
            const isResolved = ['RESOLVED', 'CLOSED'].includes(t.status?.toUpperCase());
            const deadlinePassed = t.sla_resolution_deadline && (new Date(t.sla_resolution_deadline) < new Date());
            return !isResolved && (t.sla_resolution_status === 'BREACHED' || deadlinePassed);
        }).length,
    };
        ...
        return (
            <div className="min-h-screen flex items-center justify-center p-8 bg-app-bg text-app-text">
                <div className="border border-red-500/20 bg-red-500/5 p-12 text-center max-w-md w-full">
                    ...
                    <Link href="/unit-command" className="block w-full py-4 bg-white/5 hover:bg-white/10 text-white font-bold transition-all border border-white/10 text-center text-xs uppercase tracking-widest">
                        Return to Launchpad
                    </Link>
                </div>
            </div>
    ...
    return (
        <div className="min-h-screen bg-app-bg text-app-text">
            <div className="max-w-7xl mx-auto p-6 md:p-10 space-y-8">
                ...
                <button
                    type="button"
                    onClick={() => router.push('/unit-command')}
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
                    ...
                    <StatCard label="SLA Breached" value={stats.breached}   icon={Activity}     color="red" />
                    <StatCard label="Resolved"     value={stats.resolved}   icon={CheckCircle2} color="green" />
                    <StatCard label="Live Agents"  value={12}               icon={Users}        color="primary" />
                </div>
                ...
                <div className="bg-app-surface border border-app-border overflow-hidden">
                    ...
                    <div className="p-5 border-b border-app-border flex flex-wrap items-center gap-4">
                        <h2 className="text-sm font-black uppercase tracking-widest mr-auto">
                            Ticket Queue
                            <span className="ml-3 text-primary font-mono text-xs">
                                {tickets.length < totalTickets ? `${tickets.length} of ${totalTickets}` : totalTickets}
                            </span>
                        </h2>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-app-border bg-app-surface-soft">
                                    {['Ticket ID', 'Subject', 'Requestor', 'Priority', 'Status', 'SLA', 'Created', ''].map(h => (
                    ...
                    {filteredTickets.length > 0 && (
                        <div className="px-5 py-3 border-t border-app-border flex items-center justify-between">
                            <span className="text-[9px] text-app-text-muted uppercase tracking-widest">
                                Displaying {tickets.length} of {totalTickets} total incidents
                            </span>
                            <Link
                                href={`/tickets?department=${dept?.name || ''}`}
