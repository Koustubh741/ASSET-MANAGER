...
export default function PortPolicyDetailPage() {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-50 flex items-center justify-center">
                <p className="text-app-text-muted text-sm">Loading policy…</p>
            </div>
        );
    }
    ...
    if (!policy) {
        return (
            <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-50 flex items-center justify-center">
                <p className="text-app-text-muted text-sm">Policy not found</p>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-slate-100 dark:bg-slate-950 text-slate-50">
            <header className="border-b border-slate-800 bg-slate-100 dark:bg-slate-950/80 backdrop-blur-md px-8 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/security/port-policies" className="p-2 rounded-lg bg-white dark:bg-slate-900 border border-slate-700 text-app-text-muted hover:bg-slate-100 dark:bg-slate-800 hover:text-app-text transition-colors">
                        <ArrowLeft size={18} />
                    </Link>
                    <div>
            ...
            <main className="px-8 py-6 space-y-6">
                ...
                <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="bg-white dark:bg-slate-900/70 border border-slate-800 rounded-xl p-5 lg:col-span-2">
                        <form className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm" onSubmit={handleSave}>
                            ...
                            <div>
                                <label className="block text-xs font-medium text-app-text-muted mb-1">Scope</label>
                                <select
                            ...
                            <div>
                                <label className="block text-xs font-medium text-app-text-muted mb-1">Direction</label>
                                <select
                            ...
                            <div>
                                <label className="block text-xs font-medium text-app-text-muted mb-1">Protocol</label>
                                <select
                            ...
                            <div>
                                <label className="block text-xs font-medium text-app-text-muted mb-1">Action</label>
                                <select
                            ...
                            <div>
                                <label className="block text-xs font-medium text-app-text-muted mb-1">Port</label>
                                <input
                                    type="number"
                                    value={editForm.port}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="block text-xs font-medium text-app-text-muted mb-1">Range Start</label>
                                    <input
                                        type="number"
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-app-text-muted mb-1">Range End</label>
                                    <input
                                        type="number"
                            ...
                            <div>
                                <label className="block text-xs font-medium text-app-text-muted mb-1">Priority</label>
                                <input
                                    type="number"
                            ...
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                />
                                <label htmlFor="enabled" className="text-xs text-app-text-muted">
                                    Enabled
                                </label>
                            </div>
                        </form>
                    ...
                    <div className="bg-white dark:bg-slate-900/70 border border-slate-800 rounded-xl p-5 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-sm font-semibold text-app-text">Enforcement Summary</h2>
                            <AlertTriangle size={16} className="text-app-text-muted" />
                        </div>
                        <div className="space-y-2 text-xs">
                            <div className="flex items-center justify-between">
                                <span className="text-app-text-muted">Applied</span>
                                <span className="font-mono text-emerald-300">{enforcementSummary.APPLIED}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-app-text-muted">Pending</span>
                                <span className="font-mono text-app-text-muted">{enforcementSummary.PENDING}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-app-text-muted">Failed</span>
                                <span className="font-mono text-rose-300">{enforcementSummary.FAILED}</span>
                            </div>
                            <div className="flex items-center justify-between">
                                <span className="text-app-text-muted">Rolled Back</span>
                                <span className="font-mono text-amber-300">{enforcementSummary.ROLLED_BACK}</span>
                            </div>
                        </div>
                        <div className="mt-2 pt-2 border-t border-slate-800">
                            <p className="text-[11px] text-app-text-muted">
                                Ports: <span className="text-app-text font-mono">{portsDisplay}</span>
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="bg-white dark:bg-slate-900/70 border border-slate-800 rounded-xl p-5 lg:col-span-1">
                        <h2 className="text-sm font-semibold text-app-text mb-3">Assigned Targets</h2>
                        <ul className="space-y-2 text-xs">
                            {(policy.targets || []).map((t) => (
                                ...
                                >
                                    <div>
                                        <p className="text-app-text font-mono text-[11px] truncate max-w-[200px]">
                                            {t.target_ref_id}
                                        </p>
                            ...
                            {(!policy.targets || policy.targets.length === 0) && (
                                <li className="text-app-text-muted text-xs py-2">
                                    No targets assigned. Assign an agent, host, or resource below.
                                </li>
                            )}
                        </ul>
                        <form className="mt-4 space-y-2 text-xs" onSubmit={handleAssign}>
                            <p className="text-[11px] font-semibold text-app-text-muted mb-1">
                                Assign New Target
                            </p>
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    <label className="block text-[10px] text-app-text-muted mb-1">Type</label>
                                    <select
                                        value={newTarget.target_type}
                                        onChange={(e) =>
                                            setNewTarget({ ...newTarget, target_type: e.target.value })
                                        }
                                        className="w-full rounded-lg bg-slate-100 dark:bg-slate-950 border border-slate-700 px-2 py-1.5 text-app-text text-[11px]"
                                    >
                                ...
                                <div>
                                    <label className="block text-[10px] text-app-text-muted mb-1">Scope</label>
                                    <select
                            ...
                            <div>
                                <label className="block text-[10px] text-app-text-muted mb-1">Identifier</label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-[10px] text-app-text-muted mb-1">Display Name (optional)</label>
                                <input
                                    type="text"
                            ...
                            <button
                                type="submit"
                                disabled={assigning}
                                className="mt-2 w-full inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 dark:bg-slate-800 hover:bg-slate-200 dark:bg-slate-700 text-[11px] font-semibold text-app-text disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                    ...
                    {/* Enforcement detail */}
                    <div className="bg-white dark:bg-slate-900/70 border border-slate-800 rounded-xl p-5 lg:col-span-2">
                        <h2 className="text-sm font-semibold text-app-text mb-3">Per-Agent Enforcement</h2>
                        <div className="overflow-x-auto">
                            <table className="min-w-full text-xs">
                                <thead className="bg-slate-100 dark:bg-slate-950/60 border-b border-slate-800 text-app-text-muted uppercase tracking-[0.15em]">
                                    <tr>
                                ...
                                <tbody>
                                    {enforcement.length === 0 && (
                                        <tr>
                                            <td colSpan={5} className="px-3 py-6 text-center text-app-text-muted">
                                    ...
                                    {enforcement.map((e) => (
                                        <tr key={e.id} className="border-b border-slate-800/60">
                                            <td className="px-3 py-2 text-app-text font-mono text-[11px]">
                                                {e.agent_id}
                                            </td>
                                                ...
                                                <span
                                                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${
                                                        e.status === 'APPLIED'
                                                            ...
                                                            : 'bg-slate-200 dark:bg-slate-700/40 text-app-text-muted border border-slate-500/40'
                                                    }`}
                                                >
                                                    {e.status}
                                                </span>
                                            ...
                                            <td className="px-3 py-2 text-app-text-muted max-w-xs truncate">
                                                {e.last_error || '—'}
                                            </td>
                                        </tr>
                                    ))}
