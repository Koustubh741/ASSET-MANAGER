...
export default function PortPolicyDetailPage() {
    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-slate-950 text-slate-50 flex items-center justify-center">
                <p className="text-slate-400 text-sm">Loading policy…</p>
            </div>
        );
    }
    ...
    if (!policy) {
        return (
            <div className="min-h-screen bg-slate-950 text-slate-50 flex items-center justify-center">
                <p className="text-slate-400 text-sm">Policy not found</p>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-slate-950 text-slate-50">
            <header className="border-b border-slate-800 bg-slate-950/80 backdrop-blur-md px-8 py-4 flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <Link href="/security/port-policies" className="p-2 rounded-lg bg-slate-900 border border-slate-700 text-slate-300 hover:bg-slate-800 hover:text-white transition-colors">
                        <ArrowLeft size={18} />
                    </Link>
                    <div>
            ...
            <main className="px-8 py-6 space-y-6">
                ...
                <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 lg:col-span-2">
                        <form className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm" onSubmit={handleSave}>
                            ...
                            <div className="flex items-center gap-2 mt-2">
                                ...
                                />
                                <label htmlFor="enabled" className="text-xs text-slate-300">
                                    Enabled
                                </label>
                            </div>
                        </form>
                    ...
                    <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 space-y-4">
                        <div className="flex items-center justify-between">
                            <h2 className="text-sm font-semibold text-slate-100">Enforcement Summary</h2>
                        ...
                        <div className="space-y-2 text-xs">
                            ...
                            <div className="flex items-center justify-between">
                                ...
                                <span className="font-mono text-slate-300">{enforcementSummary.PENDING}</span>
                            </div>
                            <div className="flex items-center justify-between">
                        ...
                        <div className="mt-2 pt-2 border-t border-slate-800">
                            <p className="text-[11px] text-slate-500">
                                Ports: <span className="text-slate-100 font-mono">{portsDisplay}</span>
                            </p>
                        </div>
                    </div>
                </section>
                ...
                <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    {/* Targets */}
                    <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 lg:col-span-1">
                        <h2 className="text-sm font-semibold text-slate-100 mb-3">Assigned Targets</h2>
                        <ul className="space-y-2 text-xs">
                            {(policy.targets || []).map((t) => (
                                ...
                                >
                                    <div>
                                        <p className="text-slate-100 font-mono text-[11px] truncate max-w-[200px]">
                                            {t.target_ref_id}
                                        </p>
                            ...
                            {(!policy.targets || policy.targets.length === 0) && (
                                <li className="text-slate-500 text-xs py-2">
                        ...
                        <form className="mt-4 space-y-2 text-xs" onSubmit={handleAssign}>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full rounded-lg bg-slate-950 border border-slate-700 px-2 py-1.5 text-slate-100 text-[11px]"
                                    >
                            ...
                            <div>
                                <label className="block text-[10px] text-slate-400 mb-1">Identifier</label>
                                <input
                                    type="text"
                                    required
                            ...
                            <div>
                                <label className="block text-[10px] text-slate-400 mb-1">Display Name (optional)</label>
                                <input
                                    type="text"
                            ...
                            <button
                                type="submit"
                                disabled={assigning}
                                className="mt-2 w-full inline-flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-[11px] font-semibold text-slate-100 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                    ...
                    {/* Enforcement detail */}
                    <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 lg:col-span-2">
                        <h2 className="text-sm font-semibold text-slate-100 mb-3">Per-Agent Enforcement</h2>
                        <div className="overflow-x-auto">
                            <table className="min-w-full text-xs">
                                <thead className="bg-slate-950/60 border-b border-slate-800 text-slate-400 uppercase tracking-[0.15em]">
                                    <tr>
                                ...
                                <tbody>
                                    ...
                                    {enforcement.map((e) => (
                                        <tr key={e.id} className="border-b border-slate-800/60">
                                            <td className="px-3 py-2 text-slate-100 font-mono text-[11px]">
                                                {e.agent_id}
                                            </td>
