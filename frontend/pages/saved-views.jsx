            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p className="text-slate-500 dark:text-slate-400 mt-1">Manage your personal and shared asset filters</p>
                        </div>
                    </div>
                    <Link href="/assets/search" className="btn btn-primary bg-purple-600 hover:bg-purple-500 text-slate-900 dark:text-white px-6 py-2.5 rounded-xl font-bold flex items-center gap-2 shadow-lg shadow-purple-500/20">
                        <Plus size={20} /> New View
                    </Link>
                </div>

                {/* Toolbar */}
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center glass-panel p-4 rounded-2xl bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-white/5">
                    <div className="relative w-full md:w-96">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-white/10 rounded-xl py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-purple-500/50 outline-none"
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredViews.length > 0 ? filteredViews.map(view => (
                        <div key={view.id} className="group relative bg-white dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-white/10 rounded-2xl p-6 hover:border-purple-500/30 hover:shadow-lg hover:shadow-purple-500/10 transition-all duration-300">
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <Link
                                    ...
                                    className="p-2 bg-purple-600 hover:bg-purple-500 text-slate-900 dark:text-white rounded-lg shadow-lg text-xs font-semibold flex items-center gap-1"
                                >
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-lg text-slate-900 dark:text-slate-100">{view.name}</h3>
                                    <span className="text-xs text-slate-500 dark:text-slate-400">{view.created}</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3 mb-6">
                                <div className="flex flex-wrap gap-2">
                                    {Object.entries(view.filters).map(([key, val]) => (
                                        val !== 'All' && val !== '' && (
                                            <span key={key} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-700 border border-slate-200 dark:border-white/5">
                                                <Tag size={10} className="text-slate-500 dark:text-slate-400" />
                        ...
                        <div className="col-span-full py-12 text-center">
                            <div className="p-6 inline-block rounded-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 mb-4">
                                <Filter size={48} className="text-slate-700" />
                            </div>
                            <h3 className="text-xl font-bold text-slate-500 dark:text-slate-400">No Saved Views Found</h3>
                            <p className="text-slate-500 dark:text-slate-400 mt-2 mb-6">Create a custom filter view in Smart Search to see it here.</p>
