            ...
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4">
                        <Link href="/enterprise-features" className="p-2 rounded-none hover:bg-slate-200 hover:text-slate-900 dark:hover:bg-app-surface text-app-text-muted hover:text-slate-900 dark:hover:text-white transition-colors">
                            <ArrowLeft size={24} />
                        </Link>
                        <div>
                    ...
                    <Link href="/assets/search" className="btn btn-primary bg-purple-600 hover:bg-purple-500 text-app-text px-6 py-2.5 rounded-none font-bold flex items-center gap-2 shadow-lg shadow-purple-500/20">
                        <Plus size={20} /> New View
                    </Link>
                </div>

                {/* Toolbar */}
                <div className="flex flex-col md:flex-row gap-4 justify-between items-center glass-panel p-4 rounded-none bg-white dark:bg-slate-900/50 border border-app-border">
                    <div className="relative w-full md:w-96">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-100 dark:bg-slate-950 border border-app-border rounded-none py-2.5 pl-10 pr-4 text-sm focus:ring-2 focus:ring-purple-500/50 outline-none"
                        />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredViews.length > 0 ? filteredViews.map(view => (
                        <div key={view.id} className="group relative bg-white dark:bg-slate-900/40 backdrop-blur-sm border border-app-border rounded-none p-6 hover:border-purple-500/30 hover:shadow-lg hover:shadow-purple-500/10 transition-all duration-300">
                            <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button
                                    onClick={() => handleDelete(view.id)}
                                    className="p-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-none border border-red-500/20 transition-colors"
                                    title="Delete View"
                                >
                                    <Trash2 size={16} />
                                </button>
                                <Link
                                    ...
                                    className="p-2 bg-purple-600 hover:bg-purple-500 text-app-text rounded-none shadow-lg text-xs font-semibold flex items-center gap-1"
                                >
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                <div className="p-3 bg-purple-500/10 text-purple-400 rounded-none border border-purple-500/20">
                                    <Filter size={24} />
                                </div>
                                <div>
                            ...
                            <div className="space-y-3 mb-6">
                                <div className="flex flex-wrap gap-2">
                                    {Object.entries(view.filters).map(([key, val]) => (
                                        val !== 'All' && val !== '' && (
                                            <span key={key} className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-none text-xs font-medium bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-700 border border-app-border">
